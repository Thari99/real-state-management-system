...
from language_model.NLP.nlp_service import nlpService
app = Flask(__name__)
...
#############
@app.route('/searchHouses', methods=['POST'])
def search_and_display_houses():
    try:
        # Get the search query from the request
        data = request.get_json()
        query = data.get('query', '')
        print("Searching properties", data)

        if not data or 'query' not in data:
            return jsonify({'error': 'Bad Request, query is missing'}), 400

        #nlp class instance
        nlp_service_instance = nlpService()
        properties = nlp_service_instance.search_properties(query)
        print("Extracted property id::: ", properties)
        ids = [property['id'] for property in properties]
        
        if not ids:
            return jsonify({"error": "No matching houses found"}), 404

        
        houses = House.query.filter(House.id.in_(ids)).all()  # Retrieve all houses with the provided IDs

        if not houses:
            return jsonify({"error": "No houses found"}), 404
        
        house_data = house_schema.dump(houses, many=True)  # Dump all house data
...
@app.route('/displayHotel', methods=['POST'])
def get_hotel():
    try:
        data = request.get_json()  
        ...
        hotel_data = hotel_schema.dump(hotel)  
        print("Hotel Data Type:", type(hotel_data))  
        ...
        response_data = {
            ...
            'no_of_rooms': hotel_data['no_of_rooms'],
            'land_size': hotel_data['land_size'],
            'distance': hotel_data['distance'],
            'keyWord': hotel_data['keyWord'],
            'description': hotel_data['description'],
            'upload_time': hotel_data['upload_time'],
            'images': [
                {
                    'image1': img.image1,
                    'image2': img.image2,
...
def addAgent():
    try:
        ...
        # Create new agent
        new_agent = Agent(username=agent_username, address=agent_address, NIC=agent_nic,
